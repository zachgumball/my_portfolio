...
const Hero = () => {
  const [arrows, setArrows] = useState<{ fontSize: number; left: number; top: number; type: string; animation: any }[]>([]);
  ...
  return (
    <section className="relative bg-primary h-screen flex flex-col items-center justify-center text-center p-6 overflow-hidden">
      {/* Animasi Panah yang Tidak Error */}
      {arrows.map((arrow, i) => (
        <motion.div
          key={i}
      ...
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} className="relative">
        <Image src="/profile.jpg" alt="Foto Profil Heri" width={150} height={150} className="rounded-full border-4 border-accent shadow-lg hover:scale-105 transition-transform duration-300" />
        <div className="absolute inset-0 rounded-full border-4 border-blue-400 opacity-50 animate-pulse"></div>
      </motion.div>

      {/* Nama & Deskripsi */}
      <motion.h1 initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2, duration: 0.8 }} className="text-4xl font-bold text-gray-800 mt-4">
        Halo, Saya{" "}
      ...
      <motion.p initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4, duration: 0.8 }} className="text-lg text-gray-600 mt-2">
